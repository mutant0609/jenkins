...
#surveys_df = pd.read_csv("datanew.csv")
...
with open(os.path.join(workpath,'datanew.csv'), 'r') as data:
...
for item in output:
	p="db.TaskResponse.insert("+json.dumps(item,ensure_ascii=False)+")"
	myarray.append(p)
print(myarray)
#myarray=json.dumps(output,ensure_ascii=False)
